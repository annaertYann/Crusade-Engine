...
Rectf Rectf::operator*(float x)
{
	Rectf temp{left,bottom,width,height};
	float withDiff{(width*x) - width};
	float heightDiff{ (height * x) - height };
	if (abs(withDiff) > 0.1f)
	{
		temp.left -= withDiff/2;
		temp.width += withDiff;
	}
	if (abs(heightDiff) > 0.1f)
	{
		temp.bottom -= heightDiff / 2;
		temp.height += heightDiff;
	}
	return temp;
}
//-----------------------------------------------------------------
